...
//CURL ...
func CURL(proxy string, httpReq *http.Request) ([]byte, *http.Response) {
	...
	sURL := proxy + strings.Replace(httpReq.URL.Path, "/wp/", "/", 1)
	log.Printf("sURL: %s \n", sURL)
	...
	tlsConf := &tls.Config{}
	tlsTransport := &http.Transport{TLSClientConfig: tlsConf, DisableKeepAlives: true}
	...
	switch proxyResp.Header.Get("Content-Encoding") {
	case "gzip":
		respReader, err = gzip.NewReader(proxyResp.Body)
		defer respReader.Close()
	case "":
		respReader = proxyResp.Body
	default:
		respReader = proxyResp.Body
	}
